        ...
        return (
            <div>
                <div className={`chat-group ${role}`}>
                    <Tooltip placement={role !== 'self' ? 'left' : 'right'} title={formatDatetime(message.createdAt)}>
                        <div className={`chat ${message.isError ? `faded` : ``}`}>
                            <div className="sender-name">{message.senderName}</div>
                            <span
                                className={message.content && searchMessage && message.content.toLowerCase().includes(searchMessage.toLowerCase()) && `search-matching-item search-right-message`}>
